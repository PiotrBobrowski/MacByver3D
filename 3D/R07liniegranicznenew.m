...
if nMultiple3>0 % sprawdzenie czy sa Multiple3
    ...
    for i=1:nMultiple3 % petla po Multiple3
        ...
        edgepoints=zeros(wymiary,'uint8'); % inicjalizacja tablicy do kroczenia
        rozgalezienia=zeros(10,3,'uint32'); % inicjalizacja: tablica rozgalezien y,x,z
        nrozgalezienia=0;

        % konwersja multipla
        MultipleP=Multiple3points(i,:); % pobranie punktow multipla
        ...
        for j=1:MultipleP(1)
            ...
            edgepoints(MultipleYXZ(j,3)+1,MultipleYXZ(j,4)+1,MultipleYXZ(j,5)+1)=4;
        end
        clear j;
        MultipleP(2:length(MultipleP))=[]; % pozostawiam tylko liczbe punktow
        % koniec konwersji multipla
        
        % wypelnienie tablicy do kroczenia po krawedzi
        fP33=find(boundarypoints.P33.grainno(:,1)==Multiple3grainno(i,1) ...
            & boundarypoints.P33.grainno(:,2)==Multiple3grainno(i,2) ...
            & boundarypoints.P33.grainno(:,3)==Multiple3grainno(i,3));
        for j=1:length(fP33)
            edgepoints(boundarypoints.P33.coord(fP33(j),1)+1, ...
                boundarypoints.P33.coord(fP33(j),2)+1, ...
                boundarypoints.P33.coord(fP33(j),3)+1)=3;
        end
        clear j fP33;
        % koniec wypelniania tablicy do kroczenia po krawedzi

        % petla glowna parowania Multipli
        for j=1:MultipleP % petla po wszystkich punktach w Multiplu
            if MultipleYXZ(j,1)~=0 % ktore jeszcze nie zostaly wyzerowane

                % wspolrzedne punktu poczatkowego kroczenia
                y=MultipleYXZ(j,3)+1;
                x=MultipleYXZ(j,4)+1;
                z=MultipleYXZ(j,5)+1;
                % koniec wspolrzednych punktu poczatkowego kroczenia

                % petla glowna kroczenia po granicy
                para=0;
                while para==0 % rob do znalezienia pary
                    
                    % lista mozliwych krokow
                    stepdirections=[false false false false false false]; % z-1,z+1,y-1,x+1,y+1,x-1
                    if (z>1 && edgepoints(y,x,z-1)~=0) % z-1
                        stepdirections(1)=true;
                    end
                    if (z<wymiary(3)+1 && edgepoints(y,x,z+1)~=0) % z+1
                        stepdirections(2)=true;
                    end
                    if (y>1 && edgepoints(y-1,x,z)~=0) % y-1
                        stepdirections(3)=true;
                    end
                    if (x<wymiary(2)+1 && edgepoints(y,x+1,z)~=0) % x+1
                        stepdirections(4)=true;
                    end
                    if (y<wymiary(1)+1 && edgepoints(y+1,x,z)~=0) % y+1
                        stepdirections(5)=true;
                    end
                    if (x>1 && edgepoints(y,x-1,z)~=0) % x-1
                        stepdirections(6)=true;
                    end
                    % koniec listy mozliwych krokow

                    % wybor kierunku przejscia
                    fST=find(stepdirections); % znajdz mozliwe kroki
                    lfST=length(fST); % liczba mozliwych krokow
                    if lfST==1 % zrob krok
                        edgepoints(y,x,z)=0; % wyzerowanie uzytego punktu
                    ...
                    elseif lfST>1 % zrob krok i zanotuj rozgalezienie
                        edgepoints(y,x,z)=0; % wyzerowanie uzytego punktu

                        % zapisanie rozgalezienia
                        for k=2:length(fST)
                            nrozgalezienia=nrozgalezienia+1;
                            rozgalezienia(nrozgalezienia,1:3)=[y x z]; % zapisanie rozgalezienia
                        end
                        clear k;
                        % koniec zapisu rozgalezienia                       
                    ...
                    elseif nrozgalezienia>0 % wroc do ostatniego rozgalezienia

                        y=rozgalezienia(nrozgalezienia,1);
                        x=rozgalezienia(nrozgalezienia,2);
                        z=rozgalezienia(nrozgalezienia,3);
                        rozgalezienia(nrozgalezienia,1:3)=[0 0 0];
                        nrozgalezienia=nrozgalezienia-1;

                    else % nie mozna zrobic kroku
                        break;
                    end
                    % koniec wyboru kierunku przejscia 

                    % sprawdzenie zakonczenia odcinka
                    for k=1:MultipleP % petla po punktach w Multiplu
                        if (y==MultipleYXZ(k,3)+1 && x==MultipleYXZ(k,4)+1 && z==MultipleYXZ(k,5)+1)
                            para=k;
                            break;
                        end
                ...
                clear y x z k fST lfST stepdirections;
                % koniec petli glownej kroczenia po granicy

                % zapisanie znalezionej pary
                if para>0
                    nBL3=nBL3+1;
                    BL3grainno(nBL3,1:3)=Multiple3grainno(i,1:3);
        ...
        for j=1:MultipleP
            if (MultipleYXZ(j,1)~=0)
                nSingle3=nSingle3+1;
                Single3points(nSingle3,1:2)=MultipleYXZ(j,1:2); % typ i numer singla
                Single3grainno(nSingle3,1:3)=Multiple3grainno(i,1:3); % numery ziaren singla
            end
        end
    ...
    clear i edgepoints rozgalezienia nrozgalezienia;
