...
% w razie remisu nic nie rob
tic

% global variables and external parameters
        ...
        while ChangedVoxelsTotalOld-ChangedVoxelsTotal~=0 % do while something changes
            ...
            for j=1:lVy % loop on voxels in current grain
                ...
                if nFriends<=2
                    ...
                    % find most friendly neighbor and save overwrite direction
                    mFriends2=max(nFriends2); % get max value
                    iFriends2=find(nFriends2==mFriends2);
                    if length(iFriends2)==1 % dispatch only when one most friendly neighbor
                        nvtc=nvtc+1;
                        vtc(nvtc,1:4)=[y x z iFriends2]; % save the exposed voxel to change
                    end
                    % end find most friendly neighbor and save overwrite direction
